...
async def clear_channel(tasks, channel_id):
...
async def is_channel_empty(channel: discord.TextChannel):
...
async def send_search(tasks, channel_id):
    ...
    if all([
        ...
        guild_id = get_rule('STRINGS', 'GUILD_ID')
...
async def generate_search_message(image_url, role):
    embed = discord.Embed(
